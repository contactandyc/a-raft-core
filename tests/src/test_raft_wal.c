...
MACRO_TEST(wal_append_rejects_payload_over_wal_max_payload_size) {
    clear_test_dir();
    raft_wal_t wal;
    // Note: The new safety boundary caps segs to UINT32_MAX but demands they fit headers.
...
MACRO_TEST(wal_truncate_tail_clears_offset_and_file_offset) {
    ...
    MACRO_ASSERT_TRUE(wal.offsets[3].seg_id != 0);

    MACRO_ASSERT_EQ_INT(raft_wal_truncate_tail(&wal, 2), 0);
    MACRO_ASSERT_EQ_INT(wal.max_disk_index, 1);

    MACRO_ASSERT_EQ_INT(wal.offsets[2].seg_id, 0);
    MACRO_ASSERT_EQ_INT(wal.offsets[3].seg_id, 0);
...
MACRO_TEST(wal_read_entry_rejects_offset_beyond_segment_size) {
    ...
    wal.offsets[1].offset = 2 * 1024 * 1024;
...
MACRO_TEST(wal_rotation_crash_before_header_does_not_brick_recovery) {
    ...
    raft_wal_t wal2;
    // Because the atomic rename to 0000000002.wal never occurred, init ignores the .tmp and succeeds
    MACRO_ASSERT_EQ_INT(raft_wal_init(&wal2, TEST_WAL_DIR, 1, 0), 0);
...
MACRO_TEST(wal_close_is_idempotent) {
    ...
    MACRO_ASSERT_EQ_INT(raft_wal_init(&wal, TEST_WAL_DIR, 1, 0), 0);
    raft_wal_close(&wal);
    // Double close should not double-free or crash
    raft_wal_close(&wal);
}
...
MACRO_TEST(wal_rejects_segment_size_over_uint32_offset_limit) {
    clear_test_dir();
    raft_wal_t wal;
    // Passing 5000 MB (which exceeds UINT32_MAX bytes) must be rejected mathematically
