...
raft_core_t* raft_io_boot(awal_engine_t* wal, uint64_t node_id, uint64_t* init_peers, size_t num_peers, uint64_t saved_term, uint64_t saved_vote) {
    ...
    for (uint64_t i = 1; i <= wal->max_disk_index; i++) {
        ...
        if (awal_read_entry(wal, i, &term, &type, &payload, &len)) {
            ...
            // A gap in the WAL means critical disk corruption!
            free(entries);
            return NULL;
        }
    }

    raft_core_t* core = raft_core_restore(node_id, init_peers, num_peers, saved_term, saved_vote, entries, total_entries);

    // Free the volatile RAM structs (Raft core maintains its own deep copy)
...
bool raft_io_save(awal_engine_t* wal, raft_ready_t* ready) {
    ...
    // Stream the new entries into the LZ4 ring buffer
    for (size_t i = 0; i < ready->num_entries_to_save; i++) {
        ...
        if (awal_append(wal, e->term, e->index, (uint8_t)e->type, e->data, e->data_len) != 0) {
            // FATAL: Payload exceeds AWAL_PAYLOAD_MAX.
            // Aborting the disk save prevents the core from thinking this is durable!
            return false;
        }
    }

    // Flush the batch to the OS
